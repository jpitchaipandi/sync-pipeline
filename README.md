...
Free tier spin-down (after 15 min idle) is handled by an external cron service. Render reserves its Cron Jobs for paid plans, so the project uses **cron-job.org** (free):

| Field | Value |
|---|---|
| URL | `https://<service>.onrender.com/sync/all` |
| Method | `POST` |
| Schedule | every 10 minutes |
| Headers | `Authorization: Bearer <API_SECRET>` |

The `src/cron-runner.ts` script in this repo is the same call wrapped in a Node CLI — useful for ad-hoc invocation or any CI cron (GitHub Actions, Vercel cron, etc.).
